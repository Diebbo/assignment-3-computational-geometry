...
Points QuickHull::compute(const Points &points) {
  ...
  */
  ...
  prunedPoints.clear();
  for (const auto &p : points) {
    // 2. Prune all the points above s.
    if (util::sidedness(q2, q1, p) > 0) {
      prunedPoints.push_back(p);
    }
  }

  QuickHull::QuickBottomHullRecursive(q2, q1, prunedPoints, hull);

  return hull;
}

void QuickHull::QuickBottomHullRecursive(const Point &p1, const Point &p2,
  ...
  /* 1. Find the point q below s that has the largest distance to s. */
  ...
  QuickBottomHullRecursive(p1, q, leftSet, hull);

  hull.push_back(q);

  QuickBottomHullRecursive(q, p2, rightSet, hull);
}
